...
for(samp in units$sample){
  ...
  file_1 <- paste0("raw_data/",subset(units,sample==samp)$fq1)
  if (file.exists(file_1)){
    system(
      paste0("ln -sr ",file_1," raw_data/", samp, "-R1.fastq.gz")
    )
  ...
  # merge R2 reads
  file_2 <- paste0("raw_data/",subset(units,sample==samp)$fq2)
  if (file.exists(file_1)){
    system(
      paste0("ln -sr ",file_2," raw_data/", samp, "-R2.fastq.gz")
    )
